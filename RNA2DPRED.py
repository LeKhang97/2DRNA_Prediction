...
def valid_motif(struct, pseudoknot = False):
    ...
    if len(struct.replace('&','').replace('_','')) <= 10:
        return False
...
def main_argument():
    ...
    parser.add_argument('-t',
		...
		type = float,
		default = 0.01,
        help ='Pairing probability threshold. Default = 0.01.')
...
if __name__ == "__main__":
    ...
    superpose = True
    if args.verbose:
        print("Begin to solve integer programming problem...")
    ...
    while True:
        ...
        m.add_constraints(x[i]*len(flatten(filtered_motif_dict[i]['pos'])) + x[j]*len(flatten(filtered_motif_dict[j]['pos'])) 
        <= len(set(flatten(filtered_motif_dict[i]['pos']) + flatten(filtered_motif_dict[j]['pos'])))
        for i,j in itertools.combinations(filtered_motif_dict,2)) 
        ...
        for motif in filtered_motif_dict:
            for comp in filtered_motif_dict[motif]['pos']:
                for pair in pairing_dict:
                    if ((pair[0] <= comp[0] and pair[0] >= comp[len(comp)-1]) or 
                    (pair[1] <= comp[0] and pair[1] >= comp[len(comp)-1])):
        ...
        if pkn == False:
            ...
            if args.verbose:
                print("Adding constraints to avoid pseudoknots from motifs...")
            # Avoid pseudoknot from motifs
            for motif1, motif2 in itertools.combinations(filtered_motif_dict,2):
                for i,j in itertools.product(filtered_motif_dict[motif1]['pairing_pos'], filtered_motif_dict[motif2]['pairing_pos']):
                    if not(i in pairing_dict) or not(j in pairing_dict):
                        if ((j[1] >= i[1] and j[1] <= i[0] and j[0] >= i[0]) 
                        or (i[1] >= j[1] and i[1] <= j[0] and i[0] >= j[0])):
                            m.add_constraint(x[motif1] + x[motif2] <= 1)
        ...
        if len(sol_val_list) > 0:
            if superpose == True:
                ...
                m.add_constraint(sum([x[i]*sum([len(comp)**2 for comp in filtered_motif_dict[i]['pos']]) for i in filtered_motif_dict]) >= sol_val_list[-1][0] + 0.05)
 ...
 #           print("\nNo more non-dorminated solutions!")
    ...
    print("\nThe whole Pareto set is: {0} ".format(sol_val_list))

    print("\nThe whole variable set is: {0} ".format(sol_var_list))
